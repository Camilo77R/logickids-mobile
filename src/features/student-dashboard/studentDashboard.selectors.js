...
const resolveRankingStatus = (ranking) => {
  ...
  if (!ranking.total_participantes) {
    if (ranking.scope?.sesion_estado === 'activa') {
      return {
        status: 'live',
        label: 'En vivo',
...
export const buildRankingView = (ranking) => {
  ...
  const totalParticipants = Number(ranking?.total_participantes ?? 0);

  return {
    ...EMPTY_RANKING_VIEW,
    hasRanking: totalParticipants > 0,
    headline: 'Ranking del grupo',
    ...
    top3: ranking?.top3 ?? [],
    rest: ranking?.resto ?? [],
    myPosition: ranking?.mi_posicion ?? null,
