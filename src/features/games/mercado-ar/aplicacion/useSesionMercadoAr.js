...
export const MODOS_PERSISTENCIA_MERCADO_AR = Object.freeze({
  local: 'local',
  remota: 'remota',
});

export const ESTADOS_PERSISTENCIA_MERCADO_AR = Object.freeze({
  inactiva: 'inactiva',
  iniciando: 'iniciando',
  activa: 'activa',
...
const construirPersistenciaInicial = (modo) => ({
  modo,
  estado: ESTADOS_PERSISTENCIA_MERCADO_AR.inactiva,
  sesionId: null,
  eventosPendientes: 0,
  error: null,
...
export const useSesionMercadoAr = ({ configuracion, contextoSesion }) => {
  ...
  const modoPersistencia = persistenciaRemotaHabilitada
    ? MODOS_PERSISTENCIA_MERCADO_AR.remota
    : MODOS_PERSISTENCIA_MERCADO_AR.local;
  ...
  const iniciarSesionRemota = useCallback(async (dificultadSolicitada) => {
    ...
    setPersistencia((previo) => ({
      ...previo,
      estado: ESTADOS_PERSISTENCIA_MERCADO_AR.iniciando,
      error: null,
      respuestaFinalizacion: null,
    }));
    ...
    try {
      ...
      setPersistencia((previo) => ({
        ...previo,
        estado: ESTADOS_PERSISTENCIA_MERCADO_AR.activa,
        sesionId,
        respuestaInicio,
        respuestaFinalizacion: null,
        error: null,
      ...
      setPersistencia((previo) => ({
        ...previo,
        estado: ESTADOS_PERSISTENCIA_MERCADO_AR.error,
  ...
  const registrarEventoRemoto = (evento) => {
    ...
    return encadenarOperacion(async () => {
      setPersistencia((previo) => ({
        ...previo,
        estado:
          previo.estado === ESTADOS_PERSISTENCIA_MERCADO_AR.iniciando
            ? previo.estado
            : ESTADOS_PERSISTENCIA_MERCADO_AR.sincronizando,
  ...
  const finalizarSesionRemota = (finalizacionSesion) => {
    ...
    return encadenarOperacion(async () => {
      setPersistencia((previo) => ({
        ...previo,
        estado: ESTADOS_PERSISTENCIA_MERCADO_AR.finalizando,
        error: null,
      }));
      ...
      try {
        ...
        setPersistencia((previo) => ({
          ...previo,
          estado: ESTADOS_PERSISTENCIA_MERCADO_AR.finalizada,
  ...
  const observadoresJuego = {
    alIniciarPartida: () => {
      // La ronda visible debe iniciar solo cuando el backend ya dejo lista la sesion.
    },
