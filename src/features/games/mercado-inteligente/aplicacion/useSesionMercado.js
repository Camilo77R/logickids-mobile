...
export const ESTADOS_PERSISTENCIA_MERCADO = Object.freeze({
  inactiva: 'inactiva',
  iniciando: 'iniciando',
  activa: 'activa',
...
export const useSesionMercado = ({ configuracion, contextoSesion }) => {
  ...
  const modoPersistencia = persistenciaRemotaHabilitada
    ? MODOS_PERSISTENCIA_MERCADO.remota
    : MODOS_PERSISTENCIA_MERCADO.local;
  ...
  const iniciarSesionRemota = useCallback(async (dificultadSolicitada) => {
    ...
    setPersistencia((previo) => ({
      ...previo,
      estado: ESTADOS_PERSISTENCIA_MERCADO.iniciando,
      error: null,
      respuestaFinalizacion: null,
    }));
    ...
    try {
      ...
      setPersistencia((previo) => ({
        ...previo,
        estado: ESTADOS_PERSISTENCIA_MERCADO.activa,
        sesionId,
        respuestaInicio,
        respuestaFinalizacion: null,
        error: null,
  ...
  const registrarEventoRemoto = (evento) => {
    ...
    return encadenarOperacion(async () => {
      setPersistencia((previo) => ({
        ...previo,
        estado:
          previo.estado === ESTADOS_PERSISTENCIA_MERCADO.iniciando
            ? previo.estado
            : ESTADOS_PERSISTENCIA_MERCADO.sincronizando,
  ...
  const finalizarSesionRemota = (finalizacionSesion) => {
    ...
    return encadenarOperacion(async () => {
      setPersistencia((previo) => ({
        ...previo,
        estado: ESTADOS_PERSISTENCIA_MERCADO.finalizando,
        error: null,
      }));
      ...
      try {
        ...
        setPersistencia((previo) => ({
          ...previo,
          estado: ESTADOS_PERSISTENCIA_MERCADO.finalizada,
